...
مفهوم Atomic به این معناست که هر واحد تغییر، **یک هدف مشخص و مستقل** داشته باشد.
...
در واقع Commitهای یک Branch را روی انتهای Branch دیگری دوباره قرار می‌دهد. نتیجه معمولاً یک تاریخچه خطی‌تر است، اما چون Commitها بازنویسی می‌شوند، Hash آن‌ها نیز تغییر می‌کند.
...
در واقع Commit آخر را حذف می‌کند، ولی تغییرات را Stage شده نگه می‌دارد.

```bash
git reset --hard HEAD~1
```

در واقع Commit آخر و تغییرات مربوط به آن را از Working Directory نیز حذف می‌کند.
...
ابزار Git معمولاً سه ناحیه مهم دارد:
...
مفهوم **Stage یا Index** ناحیه‌ای بین فایل‌های در حال کار و Commit است. با دستور `git add` مشخص می‌کنیم که دقیقاً کدام تغییرات قرار است وارد Commit بعدی شوند.
...
دستور **`git stash`** تغییرات Commit نشده را به‌صورت موقت کنار می‌گذارد و Working Directory را تمیز می‌کند. این دستور زمانی مفید است که در حال انجام کاری هستیم اما نیاز داریم موقتاً Branch را تغییر دهیم یا تغییر دیگری را دریافت کنیم.
...
ابزار Git برخلاف بعضی سیستم‌های Version Control که تغییر هر فایل را صرفاً به‌صورت مجموعه‌ای از Diffها در نظر می‌گیرند، وضعیت پروژه را در هر Commit به‌صورت یک **Snapshot** ثبت می‌کند.

مفهوم Snapshot را می‌توان تصویری از وضعیت فایل‌های Track شده پروژه در یک لحظه مشخص در نظر گرفت.
...
یک Commit وضعیت همان نسخه از این فایل‌ها را به‌عنوان یک Snapshot ثبت می‌کند. Commit بعدی Snapshot جدیدی از وضعیت پروژه خواهد داشت.
...
مفهوم **Local Repository** نسخه‌ای از Repository است که روی سیستم توسعه‌دهنده قرار دارد. توسعه‌دهنده می‌تواند بدون اتصال به اینترنت روی آن کار کند، Branch بسازد، Commit ایجاد کند، تاریخچه را مشاهده کند و بسیاری از عملیات Git را انجام دهد.
...
مفهوم **Remote Repository** نسخه‌ای از Repository است که روی یک Server مانند GitHub قرار دارد و برای اشتراک‌گذاری کد و همکاری اعضای تیم استفاده می‌شود.
